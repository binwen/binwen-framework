...
from binwen.middleware import MiddlewareMixin, GuardMiddleware


class FakeInMiddleware(MiddlewareMixin):
...
class FakeOutMiddleware(MiddlewareMixin):
    def before_handler(self, servicer, request, context):
...
def test_base_middleware(app):
    ...
    h = MiddlewareMixin(app, h, handler)
